...
from perspective_gap_inspect.scorers import (
    prompt_writing_scorer,
    role_assignment_scorer,
)
...
def _record_to_role_assignment_sample(record: dict) -> Sample:
    return Sample(
...
def _record_to_prompt_writing_sample(record: dict) -> Sample:
    return Sample(
...
def role_assignment() -> Task:
    ...
    return Task(
        dataset=hf_dataset(
            path=HF_DATASET,
            split="test",
            sample_fields=_record_to_role_assignment_sample,
...
def prompt_writing() -> Task:
    ...
    return Task(
        dataset=hf_dataset(
            path=HF_DATASET,
            split="test",
            sample_fields=_record_to_prompt_writing_sample,
            revision=HF_REVISION,
        ),
        solver=generate(),
